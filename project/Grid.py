...
class Grid:
    ...
    def build_grid(): # maak een de map de grid van nullen en enen: 0 als vrij en 1 als er een obstacle is
        grid = []
    ...
    def clear_area(grid, x, y, clearance):  # om een vrij 3x3 gebied te vinden zodat de bot zeker door kan gaan (zou kunnen botsen tegen zijkant als 1x1)
        grid_width = len(grid)
    ...
    def astar(start_cell, goal_cell, grid): 
        ...
        while not open_cells.empty():
            ...
            # mogelijke richtingen: horizontaal, verticaal en diagonaal
            ...
            for direction_x, direction_y in directions:
                ...
                if abs(direction_x) == 1 and abs(direction_y) == 1: # bijvoorbeeld van positie (3,3) naar (4,4)
                    aangrenzend1_x, aangrenzend1_y = neighbour_x, current_cell[1]  # horizontaal aangrenzend vakje
                    aangrenzend2_x, aangrenzend2_y = current_cell[0], neighbour_y  # verticaal aangrenzend vakje

                    if (0 <= aangrenzend1_x < grid_length and 0 <= aangrenzend1_y < grid_height and # controleerd of vakjes ernaast in de grid liggen
                        0 <= aangrenzend2_x < grid_length and 0 <= aangrenzend2_y < grid_height):

                        if grid[aangrenzend1_x][aangrenzend1_y] == 1 or grid[aangrenzend2_x][aangrenzend2_y] == 1: # het zijn obstakels
                            continue  
